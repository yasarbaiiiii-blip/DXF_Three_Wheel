...
<body>
  ...
  <script>
    ...
    // ── Rover Marker ──
    function updateRover(lat, lon, heading) {
      ...
      // Rotate vehicle
      if (heading != null) {
        var vehicle = document.getElementById('rover-vehicle');
        if (vehicle) {
      ...
      items.forEach(function(item) {
        ...
        item.lines.forEach(function(line) {
          var poly = L.polyline([
            ...
            color: item.selected ? '#ef4444' : '#6366f1',
            weight: item.selected ? 3 * item.scale : 2 * item.scale,
            opacity: 0.95,
            lineCap: 'round',
            lineJoin: 'round'
          });
    ...
    function handleMessage(event) {
      try {
        ...
        if (data.type === 'updateRover') {
          updateRover(data.lat, data.lon, data.heading);
...
export function MapView({
  ...
  const projectedPlanLines = useMemo(() => {
    ...
    for (const line of lines) {
      ...
      const weight = line.layer === "boundary" ? 3 : 2;
  ...
  useEffect(() => {
    ...
    const msgKey = `${lat}:${lon}:${heading}`;
    if (msgKey === lastRoverMsgRef.current) return;
    lastRoverMsgRef.current = msgKey;
